...
int main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        short count;
        int numBytesRcvd = recv(clientSocket, (char*)&count, 2, 0);
        if (numBytesRcvd != 2)
        {
            closesocket(clientSocket);
            continue;
        ...
        {
            char buf[16];
            numBytesRcvd = recv(clientSocket, buf, 16, 0);
            if (numBytesRcvd != 16)
            {
                closesocket(clientSocket);
                break;
            ...
            short id, t, h, p;
            int day, tm, offset;
            memcpy(&id,  buf,          sizeof(short));    offset = sizeof(short);
            ...
            printf("[%s:%d]%d,%d,%d,%d,%d,%d\n", clientName, ntohs(clientAddress.sin_port), id, day, tm, t, h, p);
        }
